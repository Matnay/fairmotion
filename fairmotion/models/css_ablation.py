...
class convSeq2Seq_ablation(nn.Module):
    """convSeq2Seq_ablation model for sequence prediction. The model uses a single convSeq2Seq_ablation module to
    ...
    Attributes:
        ...
        hidden_dim: convSeq2Seq_ablation hidden size
        dropout: Probability of an element to be zeroed
        device: Device on which to run the convSeq2Seq_ablation module
    """

    def __init__(self, input_dim, hidden_dim=256, dropout=0.1, device="cuda"):
        super(convSeq2Seq_ablation, self).__init__()
        ...
        self.remove_long_term_encoder=True
        self.remove_short_term_encoder=False

        # Remove long term encoder
        if(self.remove_long_term_encoder):
        ...
        if(self.remove_short_term_encoder):
        ...
        self.short_term_decoder = cssDecoder(
            input_dim, hidden_dim//2).to(device=device)
    ...
    def run_short_term_enc_dec(self, inputs, outputs, max_len=None, z_l_embedding=None, teacher_forcing_ratio=0):
        ...
        for t in range(1, max_len):
            ...
            if self.remove_long_term_encoder:
                output = self.short_term_decoder(z_l_embedding)
            elif self.remove_short_term_encoder:
                output = self.short_term_encoder(output)
            outputs[:, t] = output
