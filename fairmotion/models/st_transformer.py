...
class STTransformerLayer(nn.Module):
    def __init__(
        ...
        num_joints=24, window=120, parallel_attention=1, device="cuda",
    ):
...
class STTransformer(nn.Module):
    ...
    * Residual connection from input to output
    """
    def __init__(
        self, input_dim, hidden_dim=128, num_layers=1, dropout=0.1,
        attention_heads=1, feedforward_size=256, num_joints=24, window=120,
        device="cpu",
    ):
